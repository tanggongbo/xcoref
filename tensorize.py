...
class CorefDataProcessor:
    def __init__(self, config, side, language='english'):
        self.config = config
        self.language = language
        self.side = side
        if 'xlm-roberta' in config['bert_tokenizer_name_src']:
            self.tokenizer = AutoTokenizer.from_pretrained(config['bert_tokenizer_name_src'])
        else:
            if side == "source":
                self.tokenizer = AutoTokenizer.from_pretrained(config['bert_tokenizer_name_src'])
            elif side == "target":
                self.tokenizer = AutoTokenizer.from_pretrained(config['bert_tokenizer_name_trg'])
        ...
        if 0: #os.path.exists(cache_path):
            #  not use cached samples
            ...
            paths = {
                'trn': join(self.data_dir, f'{self.config["train_data"]}.{language}.{self.max_seg_len}.jsonlines'),
                'dev': join(self.data_dir, f'{self.config["dev_data"]}.{language}.{self.max_seg_len}.jsonlines'),
                'tst': join(self.data_dir, f'{self.config["test_data"]}.{language}.{self.max_seg_len}.jsonlines')
            }
            if side == "target" and config['synthetic'] and not config['dev_on_srctrg']:
                paths = {
                    'trn': join(self.data_dir, f'{self.config["train_data"]}.{language}.{self.max_seg_len}.jsonlines')
                }
    ...
    def get_tensor_examples(self):
        ...
        if self.config['synthetic'] and self.side == "target" and not self.config['dev_on_srctrg']:
            return self.tensor_samples['trn']
        else:
