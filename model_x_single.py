...
class CorefModel(nn.Module):
    def __init__(self, config, device, num_genres=None):
        ...
        self.span_emb_score_ffnn_trg_adapter = self.make_ffnn(1, [config['adapter_size']], output_size=1) \
            if not config['share_module'] else None
        ...
        self.span_width_score_ffnn_trg_adapter = self.make_ffnn(1, [config['adapter_size']], output_size=1) if \
            config['use_width_prior'] and not config['share_module'] else None
        ...
        self.coref_score_ffnn_trg_adapter = self.make_ffnn(1, [config['adapter_size']], output_size=1) \
            if config['fine_grained'] and not config['share_module'] else None
    ...
    def get_params(self, named=False):
        ...
        for name, param in self.named_parameters():
            if name.startswith('bert'):
                if self.config['freeze_bottom']:
                    if "8" not in name or "9" not in name or "10" not in name or "11" not in name:
    ...
    def forward(self, side, *input):
        return self.get_predictions_and_loss(side, *input)

    def get_span_rep(self, bert, side, is_training, input_ids, input_mask, speaker_ids, sentence_map, gold_starts,
                     gold_ends, gold_mention_cluster_map, transfer=False):
        ...
        if transfer:
            num_top_spans = int(min(conf['max_num_extracted_spans'], conf['top_span_ratio_target'] * num_words))
        else:
            num_top_spans = int(min(conf['max_num_extracted_spans'], conf['top_span_ratio'] * num_words))
        # print("max_num_mention: {}, ratio*words: {}, words: {}".
        #       format(int(conf['max_num_extracted_spans']),
        #              int(conf['top_span_ratio'] * num_words), int(num_words)))
        ...
        # print("selected_idx shape: {}".format(selected_idx.shape))
    ...
    def do_scoring(self, side, top_span_emb, top_span_starts, top_span_ends, num_top_spans,
        ...
        # print("top_pairwise_fast_scores: {}, top_antecedent_idx: {}".format(
        #     top_pairwise_fast_scores.shape, top_antecedent_idx.shape))
        ...
        if conf['fine_grained']:
            ...
            for depth in range(conf['coref_depth']):
                ...
                if not do_loss and side == "target":
                    # adapter for target side during evaluation
    ...
    def do_scoring_cross(self, top_span_emb_src, top_span_starts_src, top_span_ends_src, num_top_spans_src,
        ...
        # consider the entire pair loss, maginalized loss, following the monolingual model
        # log_norm = torch.logsumexp(top_pairwise_scores, dim=1)
        # loss = torch.sum(log_norm - max_pairwise_scores)

        return loss, top_pairwise_scores, top_antecedent_idx
    def do_scoring_cross_stric_alignment(self, top_span_emb_src, top_span_starts_src, top_span_ends_src, num_top_spans_src,
                         input_ids_src, input_mask_src, speaker_ids_src, genre_src,
                         top_span_emb_trg, top_span_starts_trg, top_span_ends_trg, num_top_spans_trg,
                         input_ids_trg, input_mask_trg, speaker_ids_trg):
        ...
        distance_start = top_span_starts_src.unsqueeze(0).transpose(0, 1).repeat(1, top_span_starts_trg.shape[0]) - \
                         top_span_starts_trg.repeat(top_span_starts_src.shape[0], 1)
        distance_end = top_span_ends_src.unsqueeze(0).transpose(0, 1).repeat(1, top_span_ends_trg.shape[0]) - \
                       top_span_ends_trg.repeat(top_span_ends_src.shape[0], 1)
        mention_width_trg = top_span_ends_trg - top_span_starts_trg
        mention_width_src = top_span_ends_src - top_span_starts_src
        mention_width_gap = mention_width_src.unsqueeze(0).transpose(0, 1).repeat(1, mention_width_trg.shape[0]) - \
                            mention_width_trg.repeat(mention_width_src.shape[0], 1)

        distance_x = torch.abs(distance_start) + torch.abs(distance_end) + torch.abs(mention_width_gap)

        antecedent_mask_x = distance_x < conf['x-lingual_antecedent_mask_threshold']
        source_span_emb = self.dropout(self.coarse_bilinear(top_span_emb_src))
        target_span_emb = self.dropout(torch.transpose(top_span_emb_trg, 0, 1))
        pairwise_coref_scores = torch.matmul(source_span_emb, target_span_emb)
        pairwise_fast_scores = pairwise_coref_scores
        ...
        if conf['use_distance_prior']:
            distance_score = torch.squeeze(
                self.antecedent_distance_score_ffnn(self.dropout(self.emb_antecedent_distance_prior.weight)), 1)
            bucketed_distance = util.bucket_x_distance(distance_x)
            antecedent_distance_score = distance_score[bucketed_distance]
            pairwise_fast_scores += antecedent_distance_score
        top_pairwise_fast_scores, top_antecedent_idx = torch.topk(pairwise_fast_scores, k=max_top_antecedents)
        top_antecedent_mask = util.batch_select(antecedent_mask_x, top_antecedent_idx,
                                                device)  # [num top spans, max top antecedents]
        top_antecedent_distance = util.batch_select(distance_x, top_antecedent_idx, device)
        ...
        if conf['fine_grained']:
            ...
            if conf['use_segment_distance']:
                ...
                seg_distance_emb = self.emb_segment_distance(top_antecedent_seg_distance)
            if conf['use_features']:  # Antecedent distance
                top_antecedent_distance = util.bucket_x_distance(top_antecedent_distance)
                top_antecedent_distance_emb = self.emb_top_antecedent_distance(top_antecedent_distance)
        ...
        # only consider scores not -inf ?
        # inf_mask = (~ torch.isinf(top_pairwise_scores)).to(torch.int)
        # top_pairwise_scores = torch.isneginf(top_pairwise_scores).to(torch.int) * top_pairwise_scores
        # print(top_pairwise_scores)
        # print("scores for loss:", float(1.0) / torch.sum(torch.sum(torch.exp(top_pairwise_scores), dim=1)))
        # loss = float(1.0) / (torch.sum(torch.sum(torch.exp(top_pairwise_scores), dim=1)) + 1)
        # print("scores for loss:", torch.sum(torch.sum(torch.log(torch.exp(top_pairwise_scores) + 1), dim=1)) + 1)
        loss = float(1.0) / (
                torch.sum(torch.sum(torch.log(torch.exp(torch.clamp(top_pairwise_scores, max=10)) + 1), dim=1)) + 1)

        return loss, top_pairwise_scores, top_antecedent_idx

    def compute_cross_coref_scores(self, top_span_emb_src, top_span_emb_trg):
        """ compute the scores between src and trg mention candidates """

        #  compute the scores in a loop, to save memory, but will take more time, sigh
        scores = torch.zeros(1, top_span_emb_trg.shape[0], device=self.device)
        for src_span in top_span_emb_src:
            scores_src = []
            src_span = torch.unsqueeze(src_span, 0).repeat(top_span_emb_trg.shape[0], 1)
            ...
            if self.config['share_module'] or not self.config['with_adapters']:
                # no adapter
                pairwise_scores_src = torch.squeeze(pairwise_scores_src, -1).reshape(1, -1)
                scores = torch.cat([scores, pairwise_scores_src], dim=0)
            else:
                # with adapter
                pairwise_scores_src = torch.squeeze(self.coref_score_ffnn_trg_adapter(pairwise_scores_src), -1).reshape(
                    1,
                    -1)
                scores = torch.cat([scores, pairwise_scores_src], dim=0)
        scores = torch.cat([scores, torch.zeros(top_span_emb_src.shape[0] + 1, 1, device=self.device)], dim=-1)
        return scores  # shape (src_span_num+1, trg_span_num+1)

    def get_predictions_and_loss(self, side, input_ids_src, input_mask_src, speaker_ids_src, sentence_len_src,
                                 genre_src,
        ...
        if do_loss:
            ...
            loss_coref = self.do_scoring(side, top_span_emb_src, top_span_starts_src, top_span_ends_src,
                                         num_top_spans_src,
                                         top_span_cluster_ids_src, top_span_mention_scores_src, do_loss,
                                         input_ids_src, input_mask_src, speaker_ids_src, genre_src)

            if conf["coref_loss_both"] or conf["with_agreement"]:
                side = "source" if conf['share_module'] else "target"
                top_span_emb_trg, top_span_starts_trg, top_span_ends_trg, num_top_spans_trg, top_span_cluster_ids_trg, \
                top_span_mention_scores_trg, speaker_ids_trg = self.get_span_rep(self.bert, side,
                                                                                 is_training_trg,
            ...
            if conf["coref_loss_both"]:
                loss_coref += self.do_scoring(side, top_span_emb_trg, top_span_starts_trg, top_span_ends_trg,
                                              num_top_spans_trg,
            ...
            if conf["with_agreement"]:
                ...
                # scores = self.compute_cross_coref_scores(top_span_emb_src, top_span_emb_trg)
                #
            ...
            if conf['transfer_zero'] and not conf['direct_transfer']:
                top_span_emb_src, top_span_starts_src, top_span_ends_src, num_top_spans_src, top_span_cluster_ids_src, \
                top_span_mention_scores_src, speaker_ids_src = \
                    self.get_span_rep(self.bert, "source", is_training_src, input_ids_src, input_mask_src,
                                      speaker_ids_src, sentence_map_src, gold_starts_src, gold_ends_src,
                                      gold_mention_cluster_map_src,
                                      transfer=False)

                top_antecedent_idx_src, top_antecedent_scores_src = \
                    self.do_scoring("source", top_span_emb_src, top_span_starts_src, top_span_ends_src,
                                    num_top_spans_src,
                                    top_span_cluster_ids_src, top_span_mention_scores_src, do_loss, input_ids_src,
                                    input_mask_src, speaker_ids_src, genre_src)

                side = "target" if conf['with_adapters'] else "source"
                top_span_emb_trg, top_span_starts_trg, top_span_ends_trg, num_top_spans_trg, top_span_cluster_ids_trg, \
                top_span_mention_scores_trg, speaker_ids_trg = \
                    self.get_span_rep(self.bert, side, is_training_trg, input_ids_trg, input_mask_trg,
                                      speaker_ids_trg, sentence_map_trg, gold_starts_trg, gold_ends_trg,
                                      gold_mention_cluster_map_trg,
                                      transfer=True)

                # scores_cross_coref = self.compute_cross_coref_scores(top_span_emb_src, top_span_emb_trg)
                # return top_span_starts_src, top_span_ends_src, top_antecedent_idx_src, top_antecedent_scores_src, \
                #        top_span_starts_trg, top_span_ends_trg, scores_cross_coref
                loss_agreement, top_pairwise_scores_cross, top_antecedent_idx_cross = \
                    self.do_scoring_cross(top_span_emb_src, top_span_starts_src, top_span_ends_src,
                                          num_top_spans_src, input_ids_src, input_mask_src,
                                          speaker_ids_src, genre_src, top_span_emb_trg,
                ...
                return top_span_starts_src, top_span_ends_src, top_antecedent_idx_src, top_antecedent_scores_src, \
                       top_span_starts_trg, top_span_ends_trg, top_antecedent_idx_cross, top_pairwise_scores_cross
            elif conf['transfer_zero'] and conf['direct_transfer']:
                # use src side learned parameters, process trg side data
                bert, is_training, input_ids, input_mask, speaker_ids, sentence_map, genre, \
                gold_starts, gold_ends, gold_mention_cluster_map = self.bert, is_training_trg, \
                                                                   input_ids_trg, input_mask_trg, speaker_ids_trg, \
                                                                   sentence_map_trg, genre_trg, gold_starts_trg, \
                                                                   gold_ends_trg, gold_mention_cluster_map_trg
                side = "target" if conf['with_adapters'] and not conf['coref_adapter_only'] else "source"
                ...
                top_span_mention_scores, speaker_ids = self.get_span_rep(bert, side, is_training, input_ids,
                                                                         ...
                                                                         transfer=False)
                side = "target" if conf['with_adapters'] and not conf['coref_adapter_except'] else "source"
                top_antecedent_idx, top_antecedent_scores = self.do_scoring(side, top_span_emb, top_span_starts,
                ...
                if side == "source":
                    ...
                    bert, is_training, input_ids, input_mask, speaker_ids, sentence_map, genre, \
                    gold_starts, gold_ends, gold_mention_cluster_map = self.bert, is_training_src, \
    ...
    def get_predicted_clusters(self, span_starts, span_ends, antecedent_idx, antecedent_scores):
        ...
        # num_mentions_with_antecedent = len([m for c in predicted_clusters for m in c])
        # print("num_mentions_with_antecedent: {}".format(num_mentions_with_antecedent))
    ...
    def update_evaluator_transfer(self, predicted_clusters, mention_to_cluster_id, gold_clusters, evaluator):
        mention_to_predicted = {m: tuple(predicted_clusters[cluster_idx])
                                for m, cluster_idx in mention_to_cluster_id.items()}
        gold_clusters = [tuple(tuple(m) for m in cluster) for cluster in gold_clusters]
        mention_to_gold = {m: cluster for cluster in gold_clusters for m in cluster}
        predicted_clusters = [tuple(c) for cluster_id, c in predicted_clusters.items()]
        evaluator.update(predicted_clusters, gold_clusters, mention_to_predicted, mention_to_gold)
        return predicted_clusters
