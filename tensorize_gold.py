...
'''
tensorize for using gold mentions
'''
...
class CorefDataProcessor:
    def __init__(self, config, side, language='english'):
        ...
        if 0: #os.path.exists(cache_path):
            ...
            paths = {
                'trn': join(self.data_dir, f'train_enzh.{language}.{self.max_seg_len}.jsonlines'),
            ...
            for split, path in paths.items():
                ...
                num_none = tensor_samples.count((None, None))
                for i in range(num_none):
                    tensor_samples.remove((None, None))
...
class Tensorizer:
    ...
    def tensorize_example(self, example, is_training):
        ...
        if len(clusters) == 0:
            # fileter such data for the setting with gold mentions
            return None, None
