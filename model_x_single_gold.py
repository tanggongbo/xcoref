...
class CorefModel(nn.Module):
    ...
    def get_span_rep(self, bert, side, is_training, input_ids, input_mask, speaker_ids, sentence_map, gold_starts,
        ...
        candidate_starts = gold_starts  # torch.unsqueeze(torch.arange(0, num_words, device=device), 1).repeat(1, self.max_span_width)
        candidate_ends = gold_ends  # candidate_starts + torch.arange(0, self.max_span_width, device=device)
        ...
        if is_training and side == "source":
            candidate_labels = gold_mention_cluster_map.to(torch.long)  # [num candidates]; non-gold span has label 0
        ...
        # # Get span score
        # span_emb_score_ffnn = self.span_emb_score_ffnn
        # candidate_mention_scores = torch.squeeze(span_emb_score_ffnn(candidate_span_emb), 1)
        # if conf['use_width_prior']:
        #     span_width_score_ffnn = self.span_width_score_ffnn
        #     width_score = torch.squeeze(span_width_score_ffnn(self.emb_span_width_prior.weight), 1)
    ...
    def do_scoring(self, side, top_span_emb, top_span_starts, top_span_ends, num_top_spans,
        ...
        if conf['fine_grained']:
            ...
            for depth in range(conf['coref_depth']):
                ...
                # print(f'monolingual slow scores: {top_pairwise_slow_scores}')
                # print(f'monolingual fast scores: {top_pairwise_fast_scores}')
                # print(f'monolingual scores: {top_pairwise_scores}')
    ...
    def do_scoring_cross(self, top_span_emb_src, top_span_starts_src, top_span_ends_src, num_top_spans_src,
                         ...
                         top_span_emb_trg, top_span_starts_trg, top_span_ends_trg,
        ...
        # Coarse pruning on each mention's antecedents
        max_top_antecedents = min(num_top_spans_src, conf['max_top_x_antecedents'])
        ...
        # if torch.isinf(torch.sum(torch.sum(torch.log(torch.exp(top_pairwise_scores) + 1), dim=1))).sum() > 0:
        #     print("scores with inf:", top_pairwise_scores)
        #     print("scores for loss:", torch.log(torch.exp(top_pairwise_scores) + 1))
        loss = float(1.0) / (torch.sum(torch.sum(torch.log(torch.exp(torch.clamp(top_pairwise_scores, max=10)) + 1), dim=1)) + 1)
        # loss = float(1.0) / (torch.sum(torch.sum(torch.tanh(torch.exp(top_pairwise_scores)), dim=1)) + 1)
        # loss = float(1.0) / (torch.sum(torch.sum(torch.exp(top_pairwise_scores) * inf_mask, dim=1)) + 1)
        # loss = float(1.0) / (torch.sum(torch.logsumexp(top_pairwise_scores, dim=1)) + 1)
    ...
    def get_predictions_and_loss(self, side, input_ids_src, input_mask_src, speaker_ids_src, sentence_len_src,
        ...
        if do_loss:
            ...
            speaker_ids_src = self.get_span_rep(self.bert, "source", is_training_src,
            ...
            loss_coref = self.do_scoring(side, top_span_emb_src, top_span_starts_src, top_span_ends_src,
                                         num_top_spans_src,
                                         top_span_cluster_ids_src, do_loss,
                                         input_ids_src, input_mask_src, speaker_ids_src, genre_src)
            ...
            if conf["coref_loss_both"] or conf["with_agreement"]:
                ...
                speaker_ids_trg = self.get_span_rep(self.bert, "target",
                                                    is_training_trg,
            ...
            if conf["coref_loss_both"]:
                loss_coref += self.do_scoring(side, top_span_emb_trg, top_span_starts_trg, top_span_ends_trg,
                                              num_top_spans_trg, do_loss,
                                              input_ids_trg, input_mask_trg, speaker_ids_trg, genre_trg)

            if conf["with_agreement"]:
                loss_agreement = self.do_scoring_cross(top_span_emb_src, top_span_starts_src, top_span_ends_src,
                                                       num_top_spans_src, input_ids_src, input_mask_src,
                                                       speaker_ids_src, genre_src, top_span_emb_trg,
                                                       top_span_starts_trg, top_span_ends_trg, input_ids_trg,
                ...
                #     # adapter
                #     pairwise_scores_src = torch.squeeze(self.coref_score_ffnn_trg_adapter(pairwise_scores_src),
                #                                         -1).reshape(1, -1)
                ...
                # num_mention = 0
                # if conf['agreement_src2trg']:
                #     # from source-side to target-side
                #     num_mention += num_spans_src
            ...
            speaker_ids = self.get_span_rep(bert, side, is_training, input_ids,
